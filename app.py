...
# --- 1. CONFIG ---
st.set_page_config(page_title="Omni-Sniper Pro (IST)", page_icon="🦅", layout="wide")
st.markdown("""
    <style>
...
def plot_fractal_chart(current, match, buy_walls, sell_walls):
    ...
    for p, v in buy_walls:
        if y_min-span < p < y_max+span: fig.add_hline(y=p, line_color="green", opacity=0.5, annotation_text="Buy")
    for p, v in sell_walls:
        if y_min-span < p < y_max+span: fig.add_hline(y=p, line_color="red", opacity=0.5, annotation_text="Sell")
...
if st.button("🚀 Analyze Market"):
    with st.spinner("Downloading Data (IST)..."):
    ...
    if book_res and candle_res:
        ...
        with tab1:
            if matches:
                # Show date in IST
                match_date = matches[0]['date'].tz_convert('Asia/Kolkata').strftime('%Y-%m-%d %H:%M')
        ...
        with tab2:
            if df_season is not None:
                ...
                with c_day: st.plotly_chart(plot_bar(d_stats, "Best Day of Week"), use_container_width=True)
                with c_hour: st.plotly_chart(plot_bar(h_stats, "Best Hour (IST)"), use_container_width=True)
                
                best_hour = h_stats.idxmax()
                st.info(f"✅ **Statistical Edge:** Historically, {sym} moves best around **{best_hour}:00 IST**.")
            else: st.error("Could not fetch seasonality data.")
                
        with tab3:
            st.subheader(f"Statistical Range ({tf_label})")
            st.write(f"95% Probability Range:")
            r1, r2 = st.columns(2)
            with r1: st.error(f"🛑 Stop Loss: ${var_95:,.2f}")
            with r2: st.success(f"🚀 Take Profit: ${upside:,.2f}")
